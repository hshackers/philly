		...
		User.model.findOne().where('email', req.body.email).exec(function(err, user) {
			...
			user.resetPassword(function(err) {
				...
				if (err) {
					...
					console.error(err);
					req.flash('error', 'Error sending reset password email. Please <a href="https://github.com/hshackers/philly/issues" class="alert-link">let&nbsp;us&nbsp;know</a> about this error');
					next();
				} else {
